...
//            .publishPercentiles();
    ...
    public class Method {
        ...
        public Method(MeterRegistry registry, String name, String label) {
            this.name = name;
            this.executionsCounter = SDK_OPERATIONS.tag("method", label).register(registry);
            this.successCounter = SDK_OPERATIONS_SUCCESS.tag("method", label).register(registry);
            this.errorCounter = code -> errorsCountersMap.computeIfAbsent(code, key ->
                    SDK_OPERATIONS_FAILTURE.tag("method", label).tag("status", key.toString()).register(registry)
            );
            this.retriesCounter = code -> retriesCountersMap.computeIfAbsent(code, key ->
                    SDK_RETRY_ATTEMPS.tag("method", label).tag("status", key.toString()).register(registry)
            );
            this.durationTimer = code -> durationTimerMap.computeIfAbsent(code, key ->
                    SDK_OPERATION_LATENCY.tag("method", label).tag("status", key.toString()).register(registry)
            );
        }
        ...
        public void measure(Runnable run) {
            ...
            try {
                ...
                durationTimer.apply(code).record(ms, TimeUnit.MILLISECONDS);
            }
        }
