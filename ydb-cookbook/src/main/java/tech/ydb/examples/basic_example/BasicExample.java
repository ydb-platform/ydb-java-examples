...
public class BasicExample implements App {
    ...
    private void selectSimple() {
        String query = String.format(
            ...
            "    $format(DateTime::FromSeconds(CAST(DateTime::ToSeconds(" +
                    "DateTime::IntervalFromDays(CAST(release_date AS Int16))) AS Uint32))) " +
                    "AS release_date\n" +
            "FROM series\n" +
            "WHERE series_id = 1;",
            database);
    ...
    public void multiStep() {
        ...
        Params params1 = Params.of("$seriesId", uint64(seriesId), "$seasonId", uint64(seasonId));

        // Execute first query to get the required values to the client.
        // Transaction control settings don't set CommitTx flag to keep transaction active
        // after query execution.
        TxControl txControl1 = TxControl.serializableRw().setCommitTx(false);
        DataQueryResult result1 = retryCtx
                .supplyResult(session -> session.executeDataQuery(query1, txControl1, params1))
                .join().expect("execute data query");
        ...
        ResultSetReader resultSet = result1.getResultSet(0);
        resultSet.next();
        ...
        TxControl txControl2 = TxControl.id(txId).setCommitTx(true);
        DataQueryResult result2 = retryCtx
                .supplyResult(session -> session.executeDataQuery(query2, txControl2, params2))
                .join().expect("execute data query");

        System.out.println("\n--[ MultiStep ]--");
        // Index of result set corresponds to its order in YQL query
        new TablePrinter(result2.getResultSet(0)).print();
    }
