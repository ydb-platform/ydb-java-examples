...
```bash
docker cp ydb-local:/ydb_data/cluster/kikimr_configs/config.yaml ./trace/ydb/ydb-config.yaml
```
...
```bash
docker compose -f trace/compose-e2e.yaml up -d --force-recreate ydb
```
